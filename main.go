...
func main() {
	...
	if portString == "" {
		log.Fatal("PORT is not bound in environment")
	}

	dbURL := os.Getenv("DB_URL")
	if dbURL == "" {
		log.Fatal("DB_URL is not bound in environment")
	}
	...
	go startScrapping(apiCfg.DB, concurrency, collectionInterval)
	...
	v1Router.Get("/posts", apiCfg.authMiddleware(apiCfg.handlerGetUserPosts))
